...
import {
  createVideo,
  deleteVideo,
  getVideoDiscoveryPage,
  getYouTubeEmbed,
  updateVideo,
  VideoMedia_V1_0,
  VideoMediaResult_V1_0,
} from "./screens.server";
...
export type YouTubePlayerControl =
  ...
  | {
      type: "reload";
      key: string;
      value: { newStart: number; newEnd: number; videoId: string };
    }
...
export default function Screens({ apiOrigin, resolution, videoMediaPromise, videoMediaPageSize, me }: Screens) {
  ...
  const playerDataRefs = useRef<Map<string, VideoMediaResult>>(
    new Map<string, VideoMediaResult>(
      videoMediaInit.map((v) => {
  ...
  const fetchVideoPage = useCallback(async () => {
    ...
    const oldestObject =
      mediaArray.length > 0
        ? mediaArray.reduce((oldest, current) => {
            ...
            case VideoOriginType.YouTube: {
              ...
              if (videoId && ytMeta) {
                ...
                if (created) {
                  ...
                  const newVideoMediaResults = new Map<string, VideoMediaResult>(
                    newVideoMediaArray.map((v) => [v.video_media_id, v]),
                  );
                  setVideoMedia(newVideoMediaResults);
                  playerDataRefs.current.set(newVideoResult.video_media_id, {
                    ...newVideoResult,
                  });
                } else {
  ...
  return (
    <>
      ...
      >
                      ...
                      <RemoteControl
                        ...
                        onNewClip={async function (newStart: number, newEnd: number) {
                          ...
                          if (updated) {
                            ...
                            youtubeController({
                              type: "reload",
                              key: newMedia.video_media_id,
                              value: {
                                newStart,
                                newEnd,
                                videoId: newMedia.media_key,
                              },
                            });
                            youtubeController({
                              type: "startPollingCurrentTime",
                              key: newMedia.video_media_id,
                            });
                          }
                        }}
                        ...
                        onNewNote={async (newNote) => {
                          ...
                          if (!updated) {
                            alert(
                              "Your note was not saved on the server because you do not have permission to edit it.",
                            );
                          }
                        }}
        ...
        <div style={{ gridRow: 3 }}>
          <Statusbar action={"laurus screens"} resolution={resolution} />
        </div>
      </div>
    </>
  );
...
function Screen({ apiOrigin, playerRefs, playerDataRefs, videoMedia, onNewMedia, onRemoteControl, me }: Screen) {
  return (
    <>
      <YoutubePlayer
        playerRefs={playerRefs}
