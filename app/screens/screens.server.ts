...
const onNotOk = (status: number) => {
  switch (status) {
    case 401: {
      console.log(UNAUTHORIZED_EDIT);
      return;
    }
    case 403: {
      console.log(FORBIDDEN_NAV);
      return;
    }
  }
};
...
export async function getVideoDiscoveryPage(baseUrl: string | undefined, size: number = 10, offset?: string) {
  try {
...
export async function getVideo(baseUrl: string | undefined, videoMediaId: string) {
  try {
