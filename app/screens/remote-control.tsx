...
export default function RemoteControl({
  ...
  const [gapSize] = useState({
    outer: Math.round(10 * resolution.factor),
    inner: Math.round(5 * resolution.factor),
  });
  ...
  const adjustEndCursor = useCallback(
    (newX: number): number => {
  ...
  const adjustStartCursor = useCallback(
    (newX: number): number => {
  ...
  const { getInverseTrackValue: getVolumeValue, getTrackCursor: getVolumeCursor } = useTrackpadState(
    paramSize.capHeight,
    volumeLimit,
  );
  ...
  useLayoutEffect(() => {
    (async () => {
      ...
      const volumeInit = 0;
      const newVolumeCursor = getVolumeCursor(volumeLimit - volumeInit, volumeTrackRef.current.clientHeight);
  ...
  useLayoutEffect(() => {
    (async () => {
      ...
      const endInit = Math.min(
        videoMedia.duration,
        Math.max(0, videoMedia.end >= videoMedia.start ? videoMedia.end : videoMedia.duration),
      );
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div style={{ padding: volumeSize.innerPadding }}>
            <ParameterSliderY
              ...
              trackBackground={
                i % 2 == 0
                  ? "linear-gradient(1deg, rgb(16, 16, 16), rgb(22, 22, 22))"
                  : "linear-gradient(1deg, rgb(53, 53, 53), rgb(59, 59, 59))"
              }
              cursor={volumeCursor}
        ...
        >
          ...
          >
            <SvgRepo
              ...
              scaleToContaier={true}
              onContainerClick={() => {
                const currentPlayTime = onRemoteControl({
                  type: "getCurrentTime",
                  key: videoMedia.video_media_id,
                });
                ...
                onNewControl({
                  ...
                  key: videoMedia.video_media_id,
                  value: {
                    newStart: currentPlayTime,
                    newEnd: videoMedia.end,
            ...
            <SvgRepo
              ...
              onContainerClick={() => {
                ...
                onNewControl({
                  ...
                  key: videoMedia.video_media_id,
                  value: {
                    newStart: videoMedia.start,
                    newEnd: currentPlayTime,
                    videoId: videoMedia.media_key,
                  },
                });
              }}
        ...
        >
          ...
          >
            <i>
              {new Date(videoMedia.timestamp).toLocaleString("en-US", {
                dateStyle: "medium",
                timeStyle: "short",
              })}
            </i>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: gapSize.inner }}>
              ...
              <div ref={endRef}>
                {videoMedia.end > -1
                  ? formatTime(videoMedia.end)
                  : videoMedia.duration >= videoMedia.start
                    ? formatTime(videoMedia.duration)
                    : ""}
              </div>
            </div>
          </div>
