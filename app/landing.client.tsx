...
function LaurusText({ scale, color }: LaurusText) {
  ...
  const [l1Ids] = useState({
    linearGradient: "l1lg",
    filter: "l1f",
    strokeLinearGradient: "l1slg",
  });
  const [a1Ids] = useState({
    linearGradient: "a1lg",
    filter: "a1f",
    strokeLinearGradient: "a1slg",
  });
  const [u1Ids] = useState({
    linearGradient: "u1lg",
    filter: "u1f",
    strokeLinearGradient: "u1slg",
  });
  const [r1Ids] = useState({
    linearGradient: "r1lg",
    filter: "r1f",
    strokeLinearGradient: "r1slg",
  });
  const [u2Ids] = useState({
    linearGradient: "u2lg",
    filter: "u2f",
    strokeLinearGradient: "u2slg",
  });
  const [s1Ids] = useState({
    linearGradient: "s1lg",
    filter: "s1f",
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div
            className={
              ...
              styles["glowing-border"] +
              " " +
              (isRunning ? "" : styles["animated-button-dark"])
            }
            onClick={async () => {
              if (isRunning) return;
              if (!email) {
