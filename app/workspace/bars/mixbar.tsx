...
export default function Mixbar() {
  ...
  const [prevEffects, setPrevEffects] = useState<LaurusEffect[]>(() => {
  ...
  const selectedMixablesCount = useMemo(() => {
    return coreState.effects.filter(
      (e) => uiState.mixableEffects.includes(e.type) && e.value.mixState === LaurusMixState.Selected,
    ).length;
  }, [coreState.effects, uiState.mixableEffects]);

  const someActiveMixables = useMemo(() => {
    return coreState.effects.some(
      (e) => uiState.mixableEffects.includes(e.type) && e.value.mixState === LaurusMixState.Active,
    );
  }, [coreState.effects, uiState.mixableEffects]);
  ...
  }, [coreState.effects, dispatch, uiState.mixableEffects, selectedEffectType, prevEffects, isSaving]);
        ...
        {selectedEffectType && !someActiveMixables && (
          <>
            ...
            >
              ...
              >
                <div className={geistMono.className} style={{ textWrap: "nowrap" }}>
                  {selectedMixablesCount}
                </div>
              </div>
              <div
                title="save mix"
              ...
              >
                <SvgRepo
                  ...
                  onContainerClick={async () => {
                    ...
                    setIsSaving(true);
                    try {
                      const snapshot: LaurusEffect[] = [...coreState.effects];
                      const newEffects: LaurusEffect[] = snapshot.map((e) => {
                        if (e.type === selectedEffectType && uiState.mixableEffects.includes(e.type)) {
                          const newMixState: LaurusMixState =
                            e.value.mixState === LaurusMixState.Selected ? LaurusMixState.Active : LaurusMixState.None;
                      ...
                      for (let i = 0; i < newEffects.length; i++) {
                        ...
                        if (e.type === selectedEffectType && uiState.mixableEffects.includes(e.type)) {
                          ...
                          switch (e.type) {
                            case "move": {
                              updated = await updateMove(coreState.apiOrigin, coreState.accessToken, e.key, {
                                ...e.value,
                              });
                              break;
                            }
                            ...
                            case "scale": {
                              updated = await updateScale(coreState.apiOrigin, coreState.accessToken, e.key, {
                                ...e.value,
                              });
                              break;
                            }
          ...
          return (
            <div
              ...
              onClick={(e) => {
                ...
                snapshot.forEach((e) => {
                  ...
                  if (e.type === newSelectedEffectType && uiState.mixableEffects.includes(e.type)) {
                    const newMixState =
                      e.value.mixState === LaurusMixState.Active ? LaurusMixState.Selected : LaurusMixState.Waiting;
                    const newEffect = (() => {
                      switch (e.type) {
                        case "move":
                          return {
              ...
              style={{
                ...
                borderRadius: 0,
                color:
                  uiState.mixableEffects.includes(selectOption) || selectOption == selectHeader
                    ? "rgb(227,227,227)"
              ...
              <div>{selectOption}</div>
              {
                <SvgRepo
                  svg={(() => {
                    switch (selectOption) {
                      case selectHeader:
                        return arrowDownwardAlt();
                      case "scale":
                        return uiState.mixableEffects.includes(selectOption) ? allOut() : allOut("rgb(67,67,67)");
                      case "move":
                        return uiState.mixableEffects.includes(selectOption)
                          ? earthquake()
