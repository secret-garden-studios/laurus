...
export default function Scalebar() {
  ...
  const saveActiveScale = useCallback(
    async (scaleX: number | undefined, scaleY: number | undefined) => {
      if (isSaving) return;
      setIsSaving(true);
  ...
  const getActiveDimensions = useCallback(
    ...
    [coreState.project, target, isMultiSelect],
  );
      ...
      >
        ...
        >
          <ParameterSliderXPlusMinus
            ...
            onCursorMove={(newCursor) => {
              if (!scaleXTrackRef.current || !widthRef.current || !heightRef.current || !scaleXLiveTitleRef.current)
                return;
        ...
        >
          <ParameterSliderXPlusMinus
            ...
            cursor={scaleYCursor}
            onCursorMove={(newCursor) => {
              if (!scaleYTrackRef.current || !heightRef.current || !widthRef.current || !scaleYLiveTitleRef.current)
                return;
