...
function calculateBoundingDeltas(meta: LaurusProjectImg | LaurusProjectSvg): {
  top: number;
  right: number;
  bottom: number;
  left: number;
...
function calculateTransformedBounds(meta: LaurusProjectImg | LaurusProjectSvg): {
  width: number;
  height: number;
  deltas: {
    top: number;
...
export function DraggableProjectImg({
  ...
  const highestOrder = useMemo(() => {
    ...
    return max;
  }, [coreState.project.imgs, coreState.project.svgs]);
